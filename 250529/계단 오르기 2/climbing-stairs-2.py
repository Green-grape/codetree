...
def get_max_coin(idx, one_jump_cnt):
    ...
    for jump in [1,2]:
        ...
        if jump==1 and one_jump_cnt<3:
            cache[idx][one_jump_cnt]=max(cache[idx][one_jump_cnt], nums[idx]+get_max_coin(idx+jump, one_jump_cnt+1))
        if jump==2:
            cache[idx][one_jump_cnt]=max(cache[idx][one_jump_cnt], nums[idx]+get_max_coin(idx+jump, one_jump_cnt))
    return cache[idx][one_jump_cnt]
