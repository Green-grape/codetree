...
def get_min_rotate(idx, rev_rotate_cnt):
    ...
    # 시계방향 회전
    cur_diff=abs((before_state[idx]+rev_rotate_cnt)%10-after_state[idx])
    dp[idx][rev_rotate_cnt]=min(dp[idx][rev_rotate_cnt], cur_diff+get_min_rotate(idx+1, rev_rotate_cnt))

    # 반시계 회전
    dp[idx][rev_rotate_cnt]=min(dp[idx][rev_rotate_cnt], cur_diff+get_min_rotate(idx+1, (rev_rotate_cnt+cur_diff)%10))
    return dp[idx][rev_rotate_cnt]
